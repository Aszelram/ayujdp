<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Jembatan Data Pangrango</title>
 <!-- Memasukkan Bootstrap CSS dari CDN -->
  <link href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.3.2/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-7Fv8jZb/MZUz/oF+JDk3NohihJpOzXZdLsEN5A3tyejYdQOSwp+BWcDHXa4BSQpl" crossorigin="anonymous">
  
  <!-- Tambahkan CSS khusus untuk desain -->
  <style>
    body 
    .card {
      box-shadow: 0 4px 8px rgba(0,0,0,0.1); /* Efek bayangan kartu */
    }
    .card-title {
      color: #333; /* Warna judul kartu */
    }
    .footer {
      background-color: #007bff; /* Sesuaikan warna latar belakang footer */
      color: #fff; /* Sesuaikan warna teks footer */
      text-align: center; /* Pusatkan teks footer */
    }
  </style>
    <style>
    .img-thumbnail:hover {
      transform: scale(1.05);
      transition: transform 0.3s ease;
    }

    .nav-link {
        font-weight: bold; /* Membuat teks di navbar lebih tebal */
    }

    .nav-link.text-warning {
        color: #ffc107 !important; /* Warna kuning untuk link dashboard */
    }

    .nav-link.text-primary {
        color: #007bff !important; /* Warna biru untuk link lainnya */
    }
    
  </style>
</head>
<body>
<nav class="navbar navbar-expand-lg navbar-light bg-white">
    <a class="navbar-brand mr-auto" href="#">
        <img src="img/logojdp.png" width="180" height="35" class="d-inline-block align-top" alt="">
    </a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav ml-auto">
        <li class="nav-item active">
          <a class="nav-link text-warning" href="index.php">Home</a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-primary" href="contact.php">Contact</a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-primary" href="registrasi.php">Registrasi</a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-primary" href="login.php">Login</a>
        </li>
      </ul>
    </div>
  </nav>
   <!-- Header -->
  <div class="container mt-5">
    <div class="row">
      <div class="col-md-6">
        <h2>Layanan Internet Cepat & Stabil</h2>
        <br>
        <h5>Kami Senantiasa Menawarkan Produk Terbaik Kepada Anda Dengan Harga Yang Kompetitif Dengan Kualitas Terbaik.</h5>
      </div>
      <div class="col-md-6">
        <img src="img/layananinternet.png" width="500" height="300" class="img-fluid" alt="Gambar Internet">
      </div>
    </div>
  </div>
  <br>

  <!-- Cart -->
<div class="container mt-5">
  <h4>PAKET LAYANAN INTERNET BROADBAND KAMI</h4>
  <br>
  <div class="row">
    <div class="col-md-4 mb-4">
      <div class="card h-100 shadow">
        <img src="img/1.jpeg" width="400" height="280" class="img-fluid img-thumbnail" alt="Gambar Internet">
        <div class="card-header">
          HOME Up to 7 Mbps
        </div>
        <div class="card-body">
          <p class="card-text">Unlimited</p>
          <p class="card-text">FUP 1500 GB </p>
          <p class="card-text">UNTUK 1-3 PENGGUNA</p>
          <p class="card-text">Rp. 165.000</p>
        </div>
      </div>
    </div>
    <div class="col-md-4 mb-4">
      <div class="card h-100 shadow">
        <img src="img/2.jpeg" width="400" height="250" class="img-fluid img-thumbnail" alt="Gambar Internet">
        <div class="card-header">
          HOME Up to 15 Mbps
        </div>
        <div class="card-body">
          <p class="card-text">Unlimited</p>
          <p class="card-text">FUP 4000 GB</p>
          <p class="card-text">UNTUK 1-10 PENGGUNA</p>
          <p class="card-text">Rp. 190.000</p>
        </div>
      </div>
    </div>
    <div class="col-md-4 mb-4">
      <div class="card h-100 shadow">
        <img src="img/3.jpeg" width="400" height="250" class="img-fluid img-thumbnail" alt="Gambar Internet">
        <div class="card-header">
          SOHO Up to 25 Mbps
        </div>
        <div class="card-body">
          <p class="card-text">Unlimited</p>
          <p class="card-text">FUP 8000 GB</p>
          <p class="card-text">UNTUK 1-20 PENGGUNA</p>
          <p class="card-text">Rp. 280.000</p>
        </div>
      </div>
    </div>
  </div>
</div>
<div class="container">
  <div class="row d-flex justify-content-center">
    <div class="col-md-10">
      <img src="img/4.jpeg" width="100%" height="100%" class="img-fluid img-thumbnail" alt="Gambar Internet">
    </div>
  </div>
</div>


    <!-- daftar jangkuan -->
<div class="container mt-5">
  <div class="row">
    <div class="col-md-6">
      <!-- Daftar Jangkauan Kami -->
      <div class="card">
        <div class="card-body">
          <h4 class="card-title text-center">Daftar Jangkauan</h4>
          <p class="card-text">
            <strong>Kecamatan Cicantayan :</strong> Desa : Cicantayan, Cijalingan, Cimahi, Cimanggis, Cisande, Hegarmanah, Lembursawah, Sukadamai<br><br>
            <strong>Kecamatan Cisaat :</strong> Desa : Babakan, Cibatu, Cibolang Kaler, Cisaat, Gunungjaya, Kutasirna, Nagrak, Padaasih, Selajambe<br><br>
            <strong>Kecamatan Kadudampit :</strong> Desa : Cikahuripan, Cipetir, Citamiang, Gede, Pangrango, Kadudampit, Muaradua, Sukamaju, Sukamanis, Undrus, Binangun<br><br>
            <strong>Kecamatan Caringin :</strong> Desa : Caringin Kulon, Caringin Wetan, Cijengkol, Cikembang, Mekarjaya, PasirdatarIndah, Seuseupan, Sukamulya, Talaga
          </p>
        </div>
      </div>
    </div>
    <div class="col-md-6">
      <!-- Tempatkan elemen lain di sini jika diperlukan -->
    </div>
  </div>
</div>
<br>
<br>

  <!-- Footer -->
  <footer class="footer mt-auto py-3">
    <div class="container">
      <span>Copyright © 2024 - PT. Jembatan Data Pangrango </span>
    </div>
  </footer>

  <!-- Memasukkan Bootstrap JS dari CDN -->
  <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.7.1/jquery.min.js" integrity="sha384-rV9CFYPtLBULj7PkhfLtTHjV2zVHGzmy8pFsdGf2yd3pJ9CspmVJNk7AAFF+Y5R6" crossorigin="anonymous"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/2.11.8/umd/popper.min.js" integrity="sha384-2nGTxOY8Jh3+hs9klxl5/C65f66f9iTjofYAn2TzFbRbYpx3eB2HtvvGLmoazFby" crossorigin="anonymous"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.3.2/js/bootstrap.bundle.min.js" integrity="sha384-P8I8m8+q9/SFboJXCFZKxyUnXUgznU3HK+rZJjqcsl4BaY8a8FDN8S8ae5C/yJiR" crossorigin="anonymous"></script>
</body>
</html>
